...
def substitute_glove(iword2idx):
    # cover to the shape of (40000, 1)
    normed_embedding = embedding/np.array([np.sqrt(np.dot(gweight,gweight)) for
                                           gweight in embedding])[:,None]
    glove_match = []
    for w,idx in word2idx.items():
        if idx >= vocab_size-nb_unknown_words and w.isalpha() and w in word2glove:
            gidx = glove_index_dict[word2glove[w]]
            gweight = glove_embedding_weights[gidx,:].copy()
            # find row in embedding that has the highest cos
            # score with gweight
            gweight /= np.sqrt(np.dot(gweight,gweight))
            score = np.dot(normed_embedding[:vocab_size-nb_unknown_words],
